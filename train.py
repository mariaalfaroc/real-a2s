...
def parse_arguments():
    ...
    parser.add_argument("--use_multirest", type=str2bool, default="False", help="Whether to use samples that contain multirest")
    parser.add_argument("--encoding", type=str, default="kern", choices=ENCODING_OPTIONS, help="Encoding type to use")
...
def main():
    ...
    XFTrain, YFTrain, XFVal, YFVal, XFTest, YFTest, XTrain_FT, YTrain_FT = load_data_from_files(config.cases_dir / args.train,\
        config.cases_dir / args.val, config.cases_dir / args.test, args.use_multirest)
    w2i, i2w = check_and_retrieveVocabulary_from_files(nameOfVoc=nameOfVoc, use_multirest=args.use_multirest, encoding=args.encoding,\
    ...
    if args.finetune == True:
        ...
        # Dictionaries
        w2i_ft, _ = check_and_retrieveVocabulary_from_files(nameOfVoc=nameOfVoc, use_multirest=args.use_multirest, encoding=args.encoding,\
            YTrain=YFTrain, YVal=YFVal, YTest=YFTest, YTrain_FT=YTrain_FT)
        ...
        # Destination paths
        model_filepath = model_filepath.parent / model_filepath.name.replace(".pt", f"_ft-{args.epochs}epochs.pt")
        logs_path = logs_path.parent / logs_path.name.replace(".csv", f"-{args.epochs}epochs.csv")
