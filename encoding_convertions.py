...
if __name__ == "__main__":
    ...
    krnParser = krnConverter(encoding="kern")
    for f in os.listdir("ICASSP-Data/krn_tenor_sax_transposed"):
        if f.endswith(".krn") and not f.startswith("."):
            ...
            print(before)
            after = krnParser.convert(src_file=f)
            print(after)
            print("--------------------------------")

    krnParser_dec = krnConverter(encoding="decoupled")
    krnParser_decDot = krnConverter(encoding="decoupled_dot")
    for f in os.listdir("ICASSP-Data/krn_tenor_sax_transposed"):
        if f.endswith(".krn") and not f.startswith("."):
            f = os.path.join("ICASSP-Data/krn_tenor_sax_transposed", f)
