...
class CTCTrainedCRNN():
    ...
    def compile(self) -> None:
        ...
        self.compute_ctc_loss = nn.CTCLoss(blank=len(self.w2i), zero_infinity=True)  # The target index cannot be blank!
    ...
    def fit(
        self,
        train_data_generator: Generator[Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor], None, None],  # Generator[yield_type, send_type, return_type],
        epochs: int,
        steps_per_epoch: int,
        ...
        XVal, XLVal = map(list, zip(*[preprocess_audio(xf, self.model.encoder.width_reduction) for xf in XFVal]))
        ...
        for epoch in range(epochs):
            ...
            # Validating
            self.model.eval()
            metrics = self.evaluate(X=XVal, XL=XLVal, Y=YVal, batch_size=batch_size, encoding=self.encoding, aux_name="", print_metrics=False)
            ...
            if metrics["ser"] < self.best_val_ser:
                print(f"SER performance improved from {self.best_val_ser:.2f} to {metrics['ser']:.2f}")
        ...
        self.model.eval()
        print(f"Evaluating best validation model (at epoch {self.best_epoch}) over test data")
        # Preprocess test data and leave in RAM
        XFTest, YFTest = test_data
        XTest, XLTest = map(list, zip(*[preprocess_audio(xf, self.model.encoder.width_reduction) for xf in XFTest]))
        YTest = [preprocess_label(yf, training=False, w2i=self.w2i, encoding=self.encoding) for yf in YFTest]
        # Evaluate
        metrics = self.evaluate(X=XTest, XL=XLTest, Y=YTest, batch_size=batch_size, encoding=self.encoding, aux_name=str(weights_path).split("/")[-2], print_random_samples=True)
        for k, v in metrics.items():
...
if __name__ == "__main__":
    ...
    model = CTCTrainedCRNN(dictionaries=({}, {}), encoding="kern", device=torch.device("cuda"))